...
POINTS_DB_PATH = os.getenv("POINTS_DB_PATH", "points.json")
SHOP_DB_PATH = os.getenv("SHOP_DB_PATH", "shop.json")
PURCHASES_DB_PATH = os.getenv("PURCHASES_DB_PATH", "purchases.json")
SHOP_LOG_CHANNEL_ID = int(os.getenv("SHOP_LOG_CHANNEL_ID", "0"))
INVITES_DB_PATH = os.getenv("INVITES_DB_PATH", "invites.json")
...
intents.guilds = True  # suffisant pour les slash + rôles
...
# ---------- Slash commands ----------
DAILY_REWARD = 10  # ajuste librement
...
async def classement_cmd(interaction: discord.Interaction, top: app_commands.Range[int, 1, 50] = 10):
    ...
    embed.set_footer(text="Utilise /addpoints et /removepoints pour modifier les scores.")
...
async def _handle_purchase(interaction: discord.Interaction, key: str):
    ...
    # Récompense + logs
    if role_name:
        role = discord.utils.get(interaction.guild.roles, name=role_name)  # type: ignore
        if role:
            ok, why = await _try_add_role(interaction.user, role, f"Achat boutique: {name}")  # type: ignore
            if ok:
                await interaction.response.send_message(
                    f"✅ Tu as acheté **{name}** pour **{cost}** pts. Rôle **{role_name}** ajouté.",
                    ephemeral=True
                )
                await increment_purchase(interaction.user.id, key)
                await _send_shop_log(interaction.guild, interaction.user, name, cost, remaining, role_name, note="Rôle ajouté")
            else:
                await interaction.response.send_message(
                    f"✅ Achat **{name}** (−{cost} pts).\n⚠️ Impossible d’ajouter **{role_name}** : {why}\nPing un admin.",
                    ephemeral=True
                )
                await increment_purchase(interaction.user.id, key)
                await _send_shop_log(interaction.guild, interaction.user, name, cost, remaining, role_name, note=f"Rôle non ajouté : {why}")
        else:
            await interaction.response.send_message(
                f"✅ Tu as acheté **{name}** pour **{cost}** pts.\n⚠️ Le rôle **{role_name}** est introuvable, ping un admin.",
                ephemeral=True
            )
            await increment_purchase(interaction.user.id, key)
            await _send_shop_log(interaction.guild, interaction.user, name, cost, remaining, role_name, note="Rôle introuvable")
    else:
        ...
        await increment_purchase(interaction.user.id, key)
        await _send_shop_log(interaction.guild, interaction.user, name, cost, remaining, None, note="Remise manuelle")
...
async def shopadmin_menu(interaction: discord.Interaction):
    ...
    class AddItemModal(discord.ui.Modal, title="Ajouter un item"):
        key = discord.ui.TextInput(
            label="Clé (unique, ex: vip, robux100)",
            min_length=1,
            max_length=40
        )
        ...
        cost = discord.ui.TextInput(
            label="Coût (points, entier ≥1)",
            placeholder="1000",
            min_length=1,
            max_length=10
        )
        role_name = discord.ui.TextInput(
            label="Rôle à donner (optionnel, nom exact)",
            required=False,
            max_length=80
        )
        max_per_user = discord.ui.TextInput(
            label="Limite par utilisateur (-1 = illimité)",
            placeholder="-1",
            min_length=1,
            max_length=10
        )
        ...
        async def on_submit(self, modal_interaction: discord.Interaction):
            ...
            async with _shop_lock:
                ...
                shop[k] = {
                    ...
                    "cost": c,
                    "role_name": (str(self.role_name).strip() or None),
    ...
    class EditItemView(View):
        ...
        @discord.ui.button(label="Associer un rôle", style=discord.ButtonStyle.secondary)
        async def set_role(self, btn_inter: discord.Interaction, _):
            key_ctx = self.key

            class RoleModal(discord.ui.Modal, title=f"Rôle pour {key_ctx}"):
                role = discord.ui.TextInput(label="Nom du rôle (laisser vide pour retirer)", required=False)

                async def on_submit(self, mi: discord.Interaction):
                    role_name = str(self.role).strip() or None
                    async with _shop_lock:
                        shop = _load_shop()
                        if key_ctx not in shop:
                            return await mi.response.send_message("❌ Clé introuvable.", ephemeral=True)
                        shop[key_ctx]["role_name"] = role_name
                        _save_shop(shop)
                    txt = f"rôle **{role_name}**" if role_name else "aucun rôle"
                    await mi.response.send_message(f"✅ `{key_ctx}` → {txt}.", ephemeral=True)

            await btn_inter.response.send_modal(RoleModal())
    ...
    class RootView(View):
        ...
        async def list_items(self, btn_inter: discord.Interaction, button):
            ...
            for key, it in shop.items():
                role_part = f" | rôle: {it['role_name']}" if it.get("role_name") else ""
...
async def on_member_join(member: discord.Member):
    ...
    if inviter_id:
        ...
        await _send_invite_log(guild, f"👋 {member.mention} via code `{code}` par <@{inviter_id}> — total **{total}**.")
    else:
